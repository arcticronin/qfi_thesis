...
A photon can be lost during propagation, collection, or detection before it gives a usable SPADE outcome.  We group these common losses into a survival probability\footnote{It is not the same $\eta$ used for the model in Eq.~\eqref{eq:optical_density_matrix_chap}, but it has the same meaning, so it feel natural to use it here} $\eta$. If the photon survives, its one-photon state remains $\rho_S$.  If it is lost, the output is the vacuum state $|\mathrm{vac}\rangle$.  The two possibilities are represented by
\begin{equation}
    \rho_{\mathrm{surv}}
...
\begin{equation}
    \rho_{\mathrm{deph}}
    =\mathcal{D}_{v}(\rho_{\mathrm{align}})
    =v\rho_{\mathrm{align}}+(1-v)\operatorname{diag}_{HG}(\rho_{\mathrm{align}})
    \qquad 0\leq v\leq1
...
\end{equation}
Here $\rho$ denotes the one-photon block that enters this stage.  The vacuum branch already produced by Eq.~\eqref{eq:spade_loss_channel} remains vacuum.  The first term is the part that passes through the apparatus.  The trace in the second term is the total probability lost from all modes.  Adding the failure state keeps the complete output normalized.  A value $\tau_j=1$ means full transmission of mode $j$, while $\tau_j=0$ means complete loss of that mode.
...
\end{align}
The accessible state gives probabilities $p_y=\operatorname{Tr}(M_y\rho_M)$.  As said, we could wait for a complete low-order experiment that would record all three outcomes.  In the present model, the pre-measurement quantum channels contain optical mixing and collection loss. The available record stores only repeated counts assigned to the coarse-grained $M_1$ result. It is a coarse-graining of the POVM.

\paragraph{Classical readout and cross-talk.}
The POVM produces a probability vector $\boldsymbol p$ with components $p_0$, $p_1$, and $p_{\mathrm{fail}}$.  Confusion between reported ports is then described by the classical stochastic channel $\boldsymbol p_{\mathrm{rep}}=R\boldsymbol p$.  The sequential fit uses the readout matrix below.
\begin{equation}
    R_{\chi}=
    \begin{pmatrix}
        1-\chi & \chi   & 0 \\
...
\end{equation}
Here $N_{\mathrm{in}}$ is the number of incident photons in one $10\,\mathrm{ms}$ acquisition, and $\eta$ is their survival probability under the stated pre-measurement convention.  Their product $T$ is the identifiable conditional throughput scale.  The repeated counts follow $n_{1,r}\sim P_{\mathrm{count}}(\mu,\boldsymbol\zeta)$, where $\boldsymbol\zeta$ contains classical dispersion or tail parameters.  Background, calibration variation, and the count law may change the final count statistics and count CFI.  They do not change the state QFI before the POVM.
...
\end{equation}
Equality holds across a fixed unitary at the state-QFI checkpoint.  Converting per-surviving-photon information into per-acquisition information requires a known photon number or throughput.  The behavior of the stochastic calibration field during an unknown-source measurement must also be defined before a final count CFI can be calculated.
...
\end{equation}
This is the QFI of the assumed pre-measurement state for estimating $d_a$.  We then calculate the measurement CFI after the stated SPADE and readout maps.  These calculations show where the modeled receiver can lose information.  They do not reconstruct experimental QFI from $n_1$.
