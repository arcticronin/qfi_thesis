...
\end{equation}
The terms with $m\neq n$ are coherences between HG modes.  They describe one displaced spatial wavefunction and do not represent coherence between the star and the planet.  Thus, mutual source incoherence and HG-basis coherence are compatible.

This distinction explains why dephasing and coherent mode mixing are meaningful parts of the channel workbench.  HG-basis dephasing can suppress the off-diagonal terms in Eq.~\eqref{eq:spade_hg_coherences_appendix}.  A direct HG population measurement is insensitive to that suppression if no later operation converts coherence into population.  If coherent mode mixing follows the dephasing, the final populations can depend on both effects.  The aggregate record can then constrain only their combined population-level action.  It cannot identify the dephasing strength, mixing angle, or mixing phase separately.

The recorded observable $n_1=n_{HG_{01}}+n_{HG_{10}}$ contains only one coarse-grained diagonal statistic at each setting.  It therefore cannot reconstruct the off-diagonal HG elements or the dominant eigenvectors required for an experimental TQFI analysis.  A TQFI value can still be calculated from a fully specified source and channel model, but it is then a conditional model result rather than a reconstruction from the counts.  Coherence-sensitive measurements in additional modal bases would be needed to test this part of the channel experimentally.
...
\begin{table}[htbp]
    \centering
    \caption{Provenance registry for the provisional workbench. The shaded row highlights the single externally reported parameter. All unshaded rows represent plausible, invented sensitivity values (or mathematically assumed symmetries) that must not be described as measured.}
    ...
    \begin{tabularx}{\textwidth}{p{4.5cm} p{2.5cm} X}
        ...
        $HG_1\to HG_0$ readout    & \makecell{$0.0035$                                                           \\ (sym.)}      & Inject first-order modes and record every output      \\
...
The workbench is modular.  A measured transfer matrix, jitter distribution, or modal transmission can replace the corresponding assumption without changing the inference and QFI pipeline. E questo e' quantum.
