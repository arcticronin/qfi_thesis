...
The experimental project began with the aim of applying TQFI to SPADE data supplied after the experiment had ended.  TQFI requires the dominant eigenvalues and eigenvectors of a parameterized state, but the record contains only aggregate first-order counts.  These counts do not determine the separate modal populations or HG-basis coherences needed to reconstruct that state.  An experimental TQFI analysis is therefore not identifiable from this record alone.
