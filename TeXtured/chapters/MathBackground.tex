...
\begin{remark}[Ontic versus Epistemic Uncertainty]
    The distinction between pure and mixed states highlights two fundamentally different types of uncertainty. A pure state represents a perfectly known system. Any unpredictability in measurement is purely intrinsic to quantum mechanics (it is an \emph{ontic} uncertainty, from greek \emph{ontos}, meaning \enquote{being}). A mixed state introduces classical uncertainty (\emph{epistemic} uncertainty, from greek \emph{episteme}, meaning \enquote{knowledge}), meaning the exact state of the system is unknown. This lack of knowledge typically arises either because the system was randomly prepared in one of several possible pure states, or because the system is entangled with an unobservable environment. Because completely different preparation methods can result in the exact same mixed state, quantities like the quantum Fisher information depend solely on the final parameterized density operator $\rho(\theta)$, rather than the specific ensemble of pure states used to create it.
\end{remark}
...
\begin{Verbatim}[fontsize=\small]
    quantum                         classical
    rho_0 --> [ channel E_theta ] --> rho(theta) --> [ POVM {M_x} ] --> p(x|theta)
\end{Verbatim}
...
\begin{definition}[Quantum Fisher Information]
    ...
    \begin{equation}
        ...
        \label{eq:qfi_def}
    \end{equation}
    \faLightbulb \hspace{5pt} The QFI therefore provides a measurement-independent upper bound on the information that can be extracted locally from the state family $\rho(\theta)$. Unlike the CFI, it does not depend on a predetermined measurement.
\end{definition}
...
\begin{remark}[Vectorized evaluation of the QFIM]
    ...
    \end{equation}
    Here, the $\operatorname{vec}$ operator flattens the partial derivative matrix into a single column vector by stacking its columns. The Kronecker product structure $\left(\rho\otimes\mathbb{I}+\mathbb{I}\otimes\rho^T\right)$ defines a superoperator, which transforms standard left- and right-sided matrix multiplications into a single large matrix acting directly on the flattened vector space. Finally, ${}^{+}$ denotes the Moore--Penrose pseudoinverse, which restricts the matrix inversion strictly to the non-zero support of the density matrix to avoid division by zero in the null space. This is the formula used for the model QFI calculation in Chapter~\ref{ch:exoplanet_experiment}.
\end{remark}
