...
\begin{remark}[Chapter takeaways]
...
Let $S$ denote the accessible $n$-qubit subsystem and $E$ the inaccessible complement of $N-n$ qubits\footnote{In the standard quantum-information notation used here, $S$ denotes the system or subsystem of interest and $E$ its inaccessible complement.}.  The parameter $h_x$ serves as the estimation parameter $\theta$ of Chapters~\ref{ch:MathBackground} and~\ref{ch:methods}.  In all scenarios, restricted access is modeled by taking the partial trace $\operatorname{Tr}_E$ over $E$.  Here, \emph{local} means that only the subsystem $S$ is accessible; it does not restrict the observer to product measurements on the qubits within $S$.

The common open-chain Hamiltonian, and the bulk part of every geometry used in the numerical studies, is the $N$-qubit TFIM defined in Eq.~\eqref{eq:tfim_hamiltonian}, with nearest-neighbor $ZZ$ coupling and a transverse field along $X$:
\begin{equation}
    H_N(h_x)=-J\sum_{i=0}^{N-2}Z_iZ_{i+1}-h_x\sum_{i=0}^{N-1}X_i.
    \label{eq:noise_tfim_hamiltonian}
\end{equation}
In each numerical subsection, $H_N(h_x)$ denotes the finite-chain Hamiltonian for the geometry stated there. The numerical results use $J=1$, so the reported field values also equal the dimensionless ratio $h_x/J$.
...
These equilibrium calculations obtain the ground and Gibbs states by exact diagonalization and matrix exponentiation. They do not use the Trotterized real-time evolution introduced as a circuit construction in Chapter~\ref{ch:methods}.
...
$h_x$: the depolarizing probability $P$ is fixed for the depolarizing family,
...
Matched purity is used as a separate diagnostic.  Writing
...
\end{equation}
The target can be the purity of a reference reduced state.  This asks a simple
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c}
        ...
        Fixed-control purity comparison      & $60$ points on $[0.1,2.5]$ & ---   & --- \\
        \bottomrule
    \end{tabular}
\end{table}
For the one-qubit Project~9 subsystem, $m$ is capped at the Hilbert-space dimension, so $m=2$. Where a result is explicitly described as a refined maximum, a bounded continuous optimization is performed within the bracket containing the coarse-grid maximum; otherwise the reported maximum is over the stated field grid.
...
\begin{table}[htbp]
    \centering
    \caption{Open-chain scaling sequence with $n=N-2$. Values and locations are refined interior maxima.}
...
\begin{table}[H]%[htbp]
    \centering
    \caption{Recovery of the full-system peak as the accessible subsystem grows at fixed open-chain size $N=8$. The full-system maximum is $10.068$ at $h_x^\star=0.719$.}
...
For this run, the ground-state \texttt{lower\_tqfi} peak is $5.019$, so the $90\%$ target used to define $\beta_\star$ is $4.517$. Interpolation gives $\beta_\star=6.216$. The table should be read as a comparison of the \emph{best sensing point} available at each inverse temperature, rather than a point-by-point comparison at one fixed field. As the Gibbs state is cooled, the local thermal response grows and its dominant feature moves from the edge of the scan towards the finite-size critical region.
...
\begin{itemize}
    ...
    \item the third isolates the fixed depolarizing channel; and
...
Critical metrological response is spatially distributed.  Partial trace removes a variable fraction of it, and the retained fraction grows strongly with accessible subsystem size. In the tested ground-state families, retaining $n=7$ of the $N=8$ open chain and $n=5$ of the $N=6$ periodic chain recovers the corresponding full-system peak QFI within the reported numerical precision.
