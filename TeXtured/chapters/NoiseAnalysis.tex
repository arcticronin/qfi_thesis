...
\noindent\emph{ASCII draft.}  The double lines stand for the ket and bra indices of each subsystem.  Closing the $E$ line denotes the partial trace, while the remaining $S$ indices define $\rho_{\mathrm{G},S}$.
...
\noindent\emph{ASCII draft.}  In the Choi-matrix view, $J(\Lambda_P^{(N)})$ represents the depolarizing channel $\Lambda_P^{(N)}$.  It is kept distinct from the channel symbol itself.  The input contraction follows Eq.~\eqref{eq:choi_channel_action}, after which the output environment is traced out.
\begin{Verbatim}[fontsize=\small]
 rho_G,N(h_x)^T ======+
                       |
                 +-----+------------------+
...
\noindent\emph{ASCII draft.}  This branch begins from a separately prepared Gibbs state.  It does not apply a thermal channel to $\rho_{\mathrm{G},N}$.
\begin{Verbatim}[fontsize=\small]
               depends on h_x and beta
                 +---------------------+
 S ==============|                     |============== S --> [ QFI / TQFI ]
                 |   rho_th,N(h_x;beta)|
 E ======+=======|                     |=======+====== E
