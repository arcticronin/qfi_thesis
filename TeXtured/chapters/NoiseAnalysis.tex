...
\end{equation}
In each numerical subsection, $H_N(h_x)$ denotes the finite-chain Hamiltonian for the stated geometry.  The numerical results use $J=1$.  The infinite-chain critical point is then $h_x/J=1$, while the finite systems studied here can have their largest response at a smaller field.
...
\begin{infobox}{What is being compared}
    ...
    \end{enumerate}
    The main comparison is not ``the same state with two kinds of noise.''  It compares three reduced states produced by three different physical histories.  The traced-out subsystem $E$ is not the external bath used to prepare a Gibbs state.  It is the finite part of the probe that the observer cannot access.
\end{infobox}
...
\begin{definition}[Prepared and reduced state families]
    ...
    \end{align}
    Here $\beta$ is the inverse temperature of the Gibbs preparation.  A larger $\beta$ means a colder state.  When the ground state is nondegenerate, the Gibbs family approaches the ground-state reference as $\beta\to\infty$.  At a degeneracy, it approaches the normalized projector onto the ground-state subspace.  We use $\rho_{\text{family},\text{system size}}$ to identify both the preparation and the number of retained qubits.
\end{definition}


\subsection{The three preparation paths}

The three local states $\rho_{\mathrm{G},S}$, $\rho_{\mathrm{dep},S}$, and $\rho_{\mathrm{th},S}$ act on the same $n$-qubit Hilbert space.  However, they are produced in different ways and need not have the same spectrum or dependence on $h_x$.  Within each preparation family, the full-system QFI is a reference ceiling for the local QFI after $E$ is discarded.

The arrows below show the physical order of each calculation.  The accompanying figures use the graphical notation of Ref.~\cite{wood2015graphical}.  They are visual representations of the same maps, not extra stages in the model.  Appendix~\ref{app:noise_graphical_calculus} explains how to read the diagrams.

\paragraph{Ground-state branch.}
The first branch prepares the ground state of the field-dependent Hamiltonian and then restricts access to $S$.
\[
...
\]
There is no added noise channel in this branch.  The global ground state is pure, but it can be entangled across the $S$--$E$ partition.  Tracing out $E$ can therefore leave a mixed state on $S$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/diagrams/diagram_encoding_hamiltonian.jpg}
    \caption{Graphical construction of the ground-state density operator.  The field $h_x$ enters the Hamiltonian and therefore the ground-state vector.  Joining the state vector to its adjoint forms the global density operator.  The outgoing wires are divided into the accessible subsystem $S$ and the discarded subsystem $E$.  The global state is not assumed to factorize across this division.}
    \label{fig:diagram_encoding_hamiltonian}
\end{figure}

\paragraph{Depolarized branch.}
The second branch applies a depolarizing channel to the full ground state before $E$ is discarded.  The channel is
\begin{equation}
    \Lambda_P^{(N)}(\rho)
...
\end{equation}
At $P=0$, the input is unchanged.  At $P=1$, it is replaced by the maximally mixed $N$-qubit state.  The channel is independent of $h_x$ when $P$ is fixed.  Since the partial trace is linear, its action on the accessible subsystem is
\begin{equation}
    \rho_{\mathrm{dep},S}(h_x;P)
...
A quantum channel can be drawn in Stinespring, Kraus, or Choi form.  These forms describe the same input--output map.  In a generic Stinespring diagram, the environment is an auxiliary system used to represent the channel.  It should not automatically be identified with the physical TFIM subsystem $E$.  For the partial-trace step itself, $E$ does denote the qubits that the observer cannot access.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/diagrams/diagram_stinespring_to_kraus.jpg}
    \caption{Generic Stinespring representation of a quantum channel.  A unitary $U$ acts on the input and an auxiliary environment prepared in a fixed state.  Closing the environment wire represents the partial trace and gives the channel output $\mathcal{E}(\rho)$.  This auxiliary environment is not automatically the inaccessible TFIM subsystem $E$.  Figure~\ref{fig:diagram_stinespring_to_kraus_extended_proof} gives the graphical reduction to Kraus form.}
...
\begin{figure}[htbp]
    ...
    \caption{Generic Choi and Kraus representations of the same channel action.  The closed wires denote index contractions, including the sum over the Kraus index.  In the drawing, $\Lambda$ denotes a Choi matrix.  In the chapter equations, $\Lambda_P^{(N)}$ denotes the depolarizing map itself, whose Choi matrix is written $J(\Lambda_P^{(N)})$.  The convention is fixed by Eq.~\eqref{eq:choi_channel_action}.}
    \label{fig:diagram_Choi_to_kraus}
\end{figure}

\paragraph{Thermal branch.}
The third branch starts from a separately prepared Gibbs state.  It does not apply a thermal channel to $\rho_{\mathrm{G},N}$.  Both the Hamiltonian and the Gibbs state depend on $h_x$, while $\beta$ is fixed during a field scan.
\[
...
\begin{infobox}{Scope of the Thermal Model}
    We assume that an external preparation process creates the full finite TFIM probe in the Gibbs state
    \[
        \rho_{\mathrm{th},N}(h_x;\beta)
        =
        \frac{e^{-\beta H_N(h_x)}}{Z(h_x,\beta)}
    \]
    The preparation process itself is not modelled.  The subsystem $E$ is the part of the prepared probe that the observer cannot access.  It is not the thermal bath, and it is separate from the auxiliary environment in a generic Stinespring representation.

    The reduced state is not generally the Gibbs state of an isolated subsystem Hamiltonian
    \[
        \operatorname{Tr}_E[\rho_{\mathrm{th},N}]
        \neq
        \frac{e^{-\beta H_S}}{\operatorname{Tr}[e^{-\beta H_S}]}
    \]
    Interactions across the $S$--$E$ boundary create correlations that remain visible in the reduced state.  This branch studies the local sensitivity produced by global Gibbs preparation.  It does not model thermalization dynamics.
\end{infobox}
...
\end{equation}
The target can be a number $\gamma_{\mathrm{target}} \in [\frac{1}{2^n},1]$, representing the purity of a reference reduced state. $2^n$ is the dimension of the system.  This asks a simple
...
\begin{figure}[H]
    ...
    \caption{Fixed-control peak comparison for the open $N=6$, $n=5$ chain.
        ...
        the sampled temperatures. It is more clear if one follows the line of the thermal points on inverse temperature, from the hotter (bluer) to the cooler (more yellow). There we can notice their purity and changes in QFI while cooling down.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/noise/project9/thermal_comparison.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figures/noise/project9/thermal_evolution.png}
...
Critical metrological response is spatially distributed.  Partial trace removes a fraction of it, and the retained fraction grows with accessible subsystem size.
