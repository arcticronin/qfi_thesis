...
\begin{figure}[htbp]
    ...
    \caption{Generic Choi and Kraus representations of the same channel action.  The closed wires denote index contractions, including the sum over the Kraus index.  In the drawing, $\Lambda$ denotes a Choi tensor.  In the chapter equations, $\Lambda_P^{(N)}$ denotes the depolarizing map itself, whose Choi matrix is written $J(\Lambda_P^{(N)})$.  The convention is fixed by Eq.~\eqref{eq:choi_channel_action}.}
