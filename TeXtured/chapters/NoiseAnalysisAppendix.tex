...
\section{Graphical calculus}

We report some diagrammatic proofs of the Stinespring representation and its equivalence to the Kraus representation.  The graphical calculus is a powerful tool for reasoning about quantum processes, and it can simplify complex algebraic manipulations, and represent quantum operations in a more intuitive and visual manner.  The diagrams we used are based on \cite{wood2015graphical}


Another convenient reference is \cite{picturing_quantum_processes_2017} that uses another notation, but the one we used is more faithful to the formulas and more intuitive in my opinion. Here a small equivalence between the two notations is shown in Fig.~\ref{fig:diagram_extra_equivalence}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/diagrams/diagram_extra_equivalence.jpg}
    \caption{Equivalence between the graphical notations used in \cite{wood2015graphical} (left) and \cite{picturing_quantum_processes_2017} (right). Both depict a quantum channel acting on a state $\rho$, and a trace operation.}
    \label{fig:diagram_extra_equivalence}
\end{figure}

Here we report the proof of the equivalence between the Stinespring representation and the Kraus representation, which is a fundamental result in quantum information theory. The Stinespring representation expresses a quantum channel as a unitary operation on a larger Hilbert space, while the Kraus representation expresses it as a sum of operators acting on the original Hilbert space.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/diagrams/diagram_stinespring_to_kraus_extended_proof.jpg}
    \caption{Equivalence between the Stinespring representation and the Kraus representation.}
