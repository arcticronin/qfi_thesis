...
assert abs(thermal.max()-4.847) < .002
assert abs(depol.max()-3.419) < .002
assert np.count_nonzero(thermal > ground) == 40
np.savetxt(OUT/'tfim_curves.csv', np.column_stack((fields, ground, thermal, depol)),
    delimiter=',', header='h_over_J,ground_local_qfi,thermal_beta5_local_qfi,depolarized_P01_local_qfi', comments='')
fig, ax = plt.subplots(figsize=(8.3, 4.3))
ax.plot(fields, ground, color=WHITE, lw=2.5, label='Ground-state preparation')
ax.plot(fields, thermal, color=TEAL, lw=3, label=r'Thermal preparation ($\beta J=5$)')
ax.fill_between(fields, ground, thermal, where=thermal>ground, color=TEAL, alpha=.13, interpolate=True)
ax.set(xlabel=r'Magnetic field $h_x/J$', ylabel=r'Local QFI (with $J=1$)', xlim=(.1, 2.5), ylim=(0, 5.5))
